...
namespace TsRandomizer.Screens.SeedSelection
{
	class SeedOptionsCollection : InventoryRelicCollection
	{
		static readonly Dictionary<int, SeedOptionInfo> Options = new Dictionary<int, SeedOptionInfo>
		{
			...
			{ 1 << 21, new SeedOptionInfo { Name = "Back to the Future", Description = "When inverted, allows using the refugee camp warp when both the Timespinner Wheel and Spindle is acquired." } },
			...
			{ 1 << 23, new SeedOptionInfo { Name = "Prism Break", Description = "Lasers blocking the Military Hangar are disabled via items, rather than by defeating bosses." } },
		};
