...
namespace TsRanodmizer.Randomisation
{
	class ItemLocationMap : IEnumerable<ItemLocation>
	{
		...
		{
			var seed = GetSeed(saveFile);
			saveFile.DataKeyInts[SeedSaveFileKey] = seed;
			return new ItemLocationRandomizer().RandonmiseItemLocations(seed, new ItemLocationMap());
		}

		static int GetSeed(GameSave saveFile)
		{
			if (saveFile.DataKeyInts.TryGetValue(SeedSaveFileKey, out int seed))
				return seed;
		...
		ItemLocationMap()
		{
			...
			var lowerLakeDesolationUpwardsShaft = highJump | ProgressionItem.TimeStop;
			...
			itemLocations = new LookupDictionairy<ItemKey, ItemLocation>(loc => loc.Key)
			{
                ...
                //local lake desolation
                new ItemLocation(1,2,1016,384, lowerLakeDesolationUpwardsShaft),
				new ItemLocation(1,11,72,240, lowerLakeDesolationBridge | ProgressionItem.AntiWeed),
				new ItemLocation(1,3,56,176, highJump | ProgressionItem.TimeStop | ProgressionItem.AntiWeed),
